...
export default function InfoPanelCountry() {
 ...
 const pieData = {
    ...
    datasets: [{
      data: [graphData.total_recovered,graphData.total_unresolved,graphData.total_deaths,graphData.total_active_cases ],
      backgroundColor: [
      '#FF6384',
      '#36A2EB',
      '#FFCE56',
...
if (viewGraph === true){
  ...
  return (
    <div className = {classes.graphContainer}>
      <button className = {classes.gobackButton} onClick = {()=>setViewGraph(false)}>Go back to table</button>
...
if (dataReceived === true ){
    ...
    return (
      <div className = {classes.infoText}>Click on the country to view Graphical representation
        <TableContainer component={Paper}>
          <Table className={classes.table} size="small" aria-label="a dense table">
            <TableHead>
              <TableRow>
                <TableCell  className={classes.tableHeader} >Country</TableCell>
                <TableCell  className={classes.tableHeader} >Total Cases</TableCell>
                <TableCell  className={classes.tableHeader} >Total Recovered</TableCell>
                <TableCell  className={classes.tableHeader} >Total Unresolved</TableCell>
                <TableCell  className={classes.tableHeader} >Total Deaths</TableCell>
                <TableCell  className={classes.tableHeader} >Total Active Cases</TableCell>
            ...
            {countryState.map((key, ind) => {
                 return (
            <TableRow className = {classes.tableRow} onClick = {()=>{viewGraphs(countryState[ind])}} key={ind}>
